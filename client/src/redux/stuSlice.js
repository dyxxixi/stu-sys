...
import { getStuListApi, delStuByIdApi, addStuInfoApi, updateStuByIdApi } from "../api/stuApi";
...
export const addStuAsync = createAsyncThunk(
  'stu/addStuAsync',
  async (payload, thunkApi) => {
    const { data } = await addStuInfoApi(payload)
    // 将这个data更新到数据仓库
...
export const editStuAsync = createAsyncThunk(
  'stu/editStuAsync',
  async (payload, thunkApi) => {
    updateStuByIdApi(payload.id, payload.stuInfo)
    ...
    editStu: (state, { payload }) => {
      for (let i = 0; i < state.stuList.length; i++) {
        if (state.stuList[i].id === ~~payload.id) {
          state.stuList.splice(i, 1, payload.stuInfo)
          break
        }
      }
    }
